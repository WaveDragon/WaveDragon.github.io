...
需要注意的是，由于输入混合耦合器在主放大器和辅助放大器的输入电压之间提供了 90° 的相移，因此它们的输出电压之间也存在 90° 的相位差，即，这些可以被视为 $V_{o,m} =|V_{o,m}| V_{o,a} =−j |V_{o,a}|$ 。
...
使用(8)–(11)，$A = (K_g G_ p)^n/(K _p P_{sat})^{n−1}$ ，而辅助子PA的输入饱和功率可导出为 $ P_{on,a} + K_pP_{sat}/K_gG_p $ ，如图3(b)所示。 辅助晶体管 $W_a$ 的宽度比主晶体管 $W_m$ 的宽度大$W_m$倍
